...
async function findByEmail(email) {
    ...
    try {
        ...
        const data = await db.get(redDoc.value.employeeId);
        data.value.id = redDoc.value.employeeId;
...
/**
 * Retrieves a employee by ID from DB.
 *
 ...
 * @throws {Error} Will throw an error if it failed to retrieve a employees from DB.
 * @api public
 */
async function find() {
    const query = db.N1qlQuery.fromString(`SELECT meta().id, avatar, firstName, lastName, email, phoneNumber, position, \`role\`, status FROM \`${db.bucketName}\` WHERE type = '${DOC_TYPE}' ORDER BY firstName`);
 ...
 * @throws {Error} Will throw an error if it failed to create a employee document in DB.
 * @api public
 */
...
/**
 * Updates a employee document in DB.
 *
 * @param {String} id - The employee id
 * @param {Object} doc - The employee document
 ...
 * @throws {Error} Will throw an error if it failed to update a employee document in DB.
 * @api public
 */
 ...
 *
 * @param {Object} doc - The employee document
 * @param {String} doc.name - The employee name
...
/**
 * Removes a employee document from DB.
 *
 * @param {String} id - The employee id
 * @throws {Error} Will throw an error if it failed to remove a employee document from DB.
 * @api public
 */
...
export {findByEmail, findByID, find, insert, update, remove};
